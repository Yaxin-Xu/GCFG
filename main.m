...
%---------- Optimization for W  -------- %
[W,V] = GCFG(X,Z1,lambda1,max_iter,miu);
...
B = (B'+ B)/2;
W2 = B - diag(diag(B));
Combine_W = sigma*W1/(max(max(W1))-min(min(W1)))+(1-sigma)*W2/(max(max(W2))-min(min(W2)));
Combine_W = max(Combine_W,0);

%%   %-------------Clustering by Louvain----------%
k = full(sum(Combine_W));
twom = sum(k);
C = @(v) Combine_W(:,v) - k'*k(v)/twom;
limit = 10000;
verbose = 1;
